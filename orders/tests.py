...
class TestOrders(TestCase):
    ...
    def setUp(self):
        ...
        self.recipe = Recipe.objects.create(id=1, category=self.category, title='django beginners', description="lorem", author=self.user,
                                            published=False, price='20.00', image='default.png', method="lorem")
    ...
    def test_user_add_order(self):
        ...
        response = self.client.post(reverse('orders:add'), {'id': 1, 'first_name': 'test', 'last_name': 'test', 'email': 'test@email.com', 'address_line_1': 'test',
                                                            'postcode': 'test', 'town_city': 'test', 'country': "GB", 'order_key': 'test',
                                                            'total_paid': 200.00}, request=self.client, basket=self.basket)
